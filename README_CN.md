...
2. **部署中转服务**  
   ...
   【使用本地内置服务】在电脑端登录页面点击“服务设置”，点击保持并重启；  
   部署完成后，打开 TopoClaw 手机版扫码（主页右上角）即可连接到对应中转服务；  
...
如果您选择自行部署，命令如下，否则在电脑应用登录页面选择本地内置服务即可（参见上方【一键安装】）
...
**A:**
不需要。常规使用场景下，TopoDesktop 已内嵌 TopoClaw 与中转服务，无需再单独部署 TopoClaw 或 customer_service 做桌面聊天。其余模块按需组合：
1. **只需桌面端体验**：仅 TopoDesktop 即可
2. **需要社交协作**：TopoDesktop + customer_service
3. **需要跨设备执行**：TopoDesktop + TopoMobile + customer_service
如需二次开发或独立调试服务，再手动启动 TopoClaw 或 customer_service（见上文 Step 1）。
